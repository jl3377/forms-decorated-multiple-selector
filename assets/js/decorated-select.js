class decoratedSelect {
    ...
    createContainers() {
        ...
        this.list.innerHTML = "Selecciona ...";
    ...
    eventAddRemove() {
        ...
        for ( let i=0 ; i<items.length; i++){     
            ...
            items[i].addEventListener('click', (evt) => {
                ...
                let itemToAdd;
                let id = evt.path[0].getAttribute('data-id'); 
                let actionValue = evt.path[0].getAttribute('data-action'); // delete and add node                                     
                
                if (evt.path[0].attributes[1].value == "add") { // add items to selected list                        
                    itemToAdd = document.querySelector(this.class+" ul li.item-"+id);                        
                    itemToAdd.setAttribute('class', 'selected item item-'+id);                                              
                    evt.path[0].setAttribute('class', 'action delete-'+id); // change action to delete
                    evt.path[0].setAttribute('data-action', 'delete'); // change action to delete
                    evt.path[0].innerHTML = "Borrar";             
                    ...
                    evt.path[0].setAttribute('class', 'action add-'+id); // change action to delete
                    evt.path[0].setAttribute('data-action', 'add'); // change action to delete
